#ifndef APEX_UTILITIES_TIME_MCU_TIME_BASE_HPP
#define APEX_UTILITIES_TIME_MCU_TIME_BASE_HPP
/**
 * @file TimeBase.hpp
 ...
 *
 * RT-safe: All query functions are O(1) noexcept. Provider implementations
 * supply the clock read: POSIX providers live in SystemClocks.hpp; firmware
 * builds a provider from a HAL tick source through the same delegate type.
 */
 ...
 */
enum class TimeStandard : uint8_t {
...
constexpr int64_t GPS_EPOCH_UNIX_SECONDS = 315964800;

/// GPS - TAI offset: GPS time = TAI - 19 seconds (fixed since GPS epoch).
constexpr int32_t GPS_TAI_OFFSET_SECONDS = 19;
 ...
 */
using TimeProviderDelegate = apex::concurrency::Delegate<uint64_t>;
 ...
 */
struct Timestamp {
  uint64_t microseconds{0};                   ///< Time value in microseconds (or cycles for CYCLE).
...
#endif // APEX_UTILITIES_TIME_MCU_TIME_BASE_HPP
