#ifndef APEX_UTILITIES_TIME_MCU_TIME_CONVERT_HPP
#define APEX_UTILITIES_TIME_MCU_TIME_CONVERT_HPP
/**
 ...
 * @note RT-safe: O(1).
 */
[[nodiscard]] inline uint64_t secondsToMicroseconds(double seconds) noexcept {
  return (seconds >= 0.0) ? static_cast<uint64_t>(seconds * 1000000.0) : 0;
}
 ...
 * @note RT-safe: O(1).
 */
[[nodiscard]] inline double microsecondsToSeconds(uint64_t us) noexcept {
 ...
 * @note RT-safe: O(1).
 */
[[nodiscard]] inline uint32_t secondsToCycles(double seconds, uint32_t freqHz) noexcept {
  return (seconds >= 0.0 && freqHz > 0)
             ? static_cast<uint32_t>(seconds * static_cast<double>(freqHz))
             : 0;
}
 ...
 * @note RT-safe: O(1).
 */
[[nodiscard]] inline uint64_t cyclesToMicroseconds(uint32_t cycles, uint32_t freqHz) noexcept {
  if (freqHz == 0) {
    return 0;
  }
  return (static_cast<uint64_t>(cycles) * 1000000ULL) / freqHz;
}
 ...
 * @note RT-safe: O(1).
 */
[[nodiscard]] inline uint32_t microsecondsToCycles(uint64_t us, uint32_t freqHz) noexcept {
  return static_cast<uint32_t>((us * freqHz) / 1000000ULL);
}
 ...
 * @note RT-safe: O(1).
 */
[[nodiscard]] inline uint64_t utcToTai(uint64_t utcMicros,
                                       int32_t taiUtcOffset = TAI_UTC_OFFSET_SECONDS) noexcept {
  return utcMicros + static_cast<uint64_t>(taiUtcOffset) * 1000000ULL;
}
 ...
 * @note RT-safe: O(1).
 */
[[nodiscard]] inline uint64_t taiToUtc(uint64_t taiMicros,
                                       int32_t taiUtcOffset = TAI_UTC_OFFSET_SECONDS) noexcept {
  const uint64_t OFFSET = static_cast<uint64_t>(taiUtcOffset) * 1000000ULL;
 ...
 * @note RT-safe: O(1).
 */
[[nodiscard]] inline uint64_t taiToGps(uint64_t taiMicros) noexcept {
  const uint64_t GPS_OFFSET = static_cast<uint64_t>(GPS_TAI_OFFSET_SECONDS) * 1000000ULL;
  const uint64_t EPOCH_OFFSET = static_cast<uint64_t>(GPS_EPOCH_UNIX_SECONDS) * 1000000ULL;
  const uint64_t TOTAL_OFFSET = GPS_OFFSET + EPOCH_OFFSET;
 ...
 * @note RT-safe: O(1).
 */
[[nodiscard]] inline uint64_t gpsToTai(uint64_t gpsMicros) noexcept {
  return gpsMicros + static_cast<uint64_t>(GPS_TAI_OFFSET_SECONDS) * 1000000ULL +
         static_cast<uint64_t>(GPS_EPOCH_UNIX_SECONDS) * 1000000ULL;
}
 ...
 * @note RT-safe: O(1).
 */
[[nodiscard]] inline uint64_t utcToGps(uint64_t utcMicros,
                                       int32_t taiUtcOffset = TAI_UTC_OFFSET_SECONDS) noexcept {
...
#endif // APEX_UTILITIES_TIME_MCU_TIME_CONVERT_HPP
