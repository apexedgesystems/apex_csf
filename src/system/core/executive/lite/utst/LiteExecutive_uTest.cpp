/**
 * @file LiteExecutive_uTest.cpp
 * @brief Unit tests for LiteExecutive.
 */

#include "src/system/core/executive/lite/inc/LiteExecutive.hpp"
#include "src/system/core/executive/lite/inc/FreeRunningSource.hpp"

#include <gtest/gtest.h>

using executive::RunResult;
using executive::lite::FreeRunningSource;
using LiteExecutive = executive::lite::LiteExecutive<>;
...
TEST(LiteExecutive_DefaultConstruction, InitialState) {
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100);

  EXPECT_EQ(exec.componentId(), 0);
  EXPECT_STREQ(exec.componentName(), "LiteExecutive");
...
TEST(LiteExecutive_Lifecycle, InitSucceeds) {
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100);
...
TEST(LiteExecutive_Lifecycle, InitFailsNullTickSource) {
  LiteExecutive exec(nullptr, 100);
...
TEST(LiteExecutive_Lifecycle, InitIdempotent) {
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100);
  (void)exec.init();
...
TEST(LiteExecutive_Lifecycle, ResetClearsState) {
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100, 10);
  (void)exec.init();
  (void)exec.run();
...
TEST(LiteExecutive_Execution, RunFailsIfNotInitialized) {
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100);
...
TEST(LiteExecutive_Execution, RunsMaxCycles) {
  resetTestState();
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100, 50);
...
TEST(LiteExecutive_Execution, ShutdownStopsRun) {
  resetTestState();
  FreeRunningSource tickSource(100);
  ...
  auto shutdownTask = [](void* ctx) noexcept {
    ++callCount;
    if (callCount >= 10) {
      static_cast<LiteExecutive*>(ctx)->shutdown();
    }
  };

  LiteExecutive exec(&tickSource, 100, 1000);
...
TEST(LiteExecutive_Execution, ShutdownSetsFlag) {
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100);
  (void)exec.init();
...
TEST(LiteExecutive_Accessors, TickSourceAccessor) {
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100);
...
TEST(LiteExecutive_Accessors, SchedulerAccessor) {
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100);
...
TEST(LiteExecutive_Accessors, SetMaxCycles) {
  resetTestState();
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100, 100);
  exec.addTask({testTask, nullptr, 1, 1, 0, 0, 1});
...
/** @test LiteExecutive implements IExecutive interface. */
TEST(LiteExecutive_Interface, ImplementsIExecutive) {
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100, 10);
...
TEST(LiteExecutive_FastForward, SetsFlag) {
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100);
...
TEST(LiteExecutive_FastForward, ResetClearsFastForward) {
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100, 10);
  (void)exec.init();
  exec.setFastForward(true);
...
TEST(LiteExecutive_FastForward, RunsAllCycles) {
  resetTestState();
  FreeRunningSource tickSource(100);
  LiteExecutive exec(&tickSource, 100, 200);
  exec.addTask({testTask, nullptr, 1, 1, 0, 0, 1});
  (void)exec.init();
  exec.setFastForward(true);
...
TEST(LiteExecutive_Integration, MultipleTasksExecution) {
