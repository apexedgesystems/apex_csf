...
/** @test WATCHPOINT_TABLE_SIZE constant is 8. */
TEST(DataWatchpoint, TableSizeConstant) { EXPECT_EQ(system_core::data::WATCHPOINT_TABLE_SIZE, 8U); }
...
/** @test WATCHPOINT_GROUP_TABLE_SIZE and WATCHPOINT_GROUP_MAX_REFS constants. */
TEST(DataWatchpoint, GroupConstants) {
  EXPECT_EQ(system_core::data::WATCHPOINT_GROUP_TABLE_SIZE, 4U);
  EXPECT_EQ(system_core::data::WATCHPOINT_GROUP_MAX_REFS, 4U);
}
...
TEST(DataWatchpoint, GroupOutOfBoundsIndex) {
  ...
  WatchpointGroup group{};
  group.indices = {0, 5, 0, 0}; // Index 5 is out of bounds (table size 2)
  group.count = 2;
  group.logic = GroupLogic::AND;

  // Index 5 resolves to false, so AND fails
  EXPECT_FALSE(evaluateGroup(group, table, 2));
}
...
TEST(DataWatchpoint, GroupCustomDelegate) {
  ...
  group.indices = {0, 1, 0, 0};
  group.count = 2;
  group.logic = GroupLogic::AND; // Would normally be false (index 1 is false)
