...
TEST(SequenceValidation, SimpleRtsValid) {
  ...
  seq.steps[0].action.actionType = ActionType::DATA_WRITE;
...
/** @test DATA_WRITE with fullUid=0 is fine (targets can be whole-block). */
TEST(SequenceValidation, DataWriteZeroUidOk) {
  DataSequence seq{};
  seq.stepCount = 1;
  seq.steps[0].action.actionType = ActionType::DATA_WRITE;
