...
constexpr std::size_t SEQUENCE_MAX_STEPS = 16;

/// Maximum concurrent sequences.
constexpr std::size_t SEQUENCE_TABLE_SIZE = 8;
...
struct SequenceStep {
  ...
  std::uint8_t gotoStep{0}; ///< Target for GOTO_STEP or slot for START_RTS.
 ...
 * @param[out] startSlot Set to slot index if onComplete is START_RTS, else 0xFF.
 * @note RT-safe: O(1).
 *
 * Applies the step's onComplete action: NEXT (linear), GOTO_STEP (branch),
 * or START_RTS (trigger another sequence slot). For START_RTS, the caller
 * is responsible for starting the target sequence.
 */
inline void advanceStep(DataSequence& seq, std::uint8_t& startSlot) noexcept {
  startSlot = 0xFF;
...
/**
 * @brief Advance to the next step (simple overload for backward compatibility).
...
[[nodiscard]] inline bool isRunning(const DataSequence& seq) noexcept {
  return seq.status == SequenceStatus::WAITING || seq.status == SequenceStatus::EXECUTING ||
         seq.status == SequenceStatus::WAITING_CONDITION;
}
