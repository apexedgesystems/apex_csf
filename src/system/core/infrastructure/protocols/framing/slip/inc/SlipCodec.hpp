/**
 * @file SlipCodec.hpp
 * @brief Templated SLIP codec with owned buffers for zero-allocation framing.
 *
 * Convenience wrapper around the stateless SLIP encode/decode API. Owns the
 * decode state, decode config, and output buffers -- sized at compile time via
 * a template parameter. The raw API in SLIPFraming.hpp stays available for
...
#ifndef APEX_PROTOCOLS_SLIP_CODEC_HPP
#define APEX_PROTOCOLS_SLIP_CODEC_HPP

#include "SLIPFraming.hpp"
...
/**
 * @class SlipCodec
 * @brief SLIP codec with compile-time-sized owned buffers.
 *
 ...
 */
template <size_t MaxFrameSize = DEFAULT_MAX_FRAME_SIZE> class SlipCodec {
public:
  /// Worst-case SLIP-encoded size: every byte escapes + leading/trailing END.
  static constexpr size_t ENCODE_BUF_SIZE = MaxFrameSize * 2 + 2;
   ...
   */
  SlipCodec() noexcept { cfg_.maxFrameSize = MaxFrameSize; }
  ...
  /**
   * @brief SLIP-encode a payload into the owned encode buffer.
   *
   ...
   * @note RT-safe.
   */
  IoResult encode(apex::compat::bytes_span payload, bool leadingEnd = true,
                  bool trailingEnd = true) noexcept {
    return slip::encode(payload, encodeBuf_, ENCODE_BUF_SIZE, leadingEnd, trailingEnd);
  }
...
#endif // APEX_PROTOCOLS_SLIP_CODEC_HPP
