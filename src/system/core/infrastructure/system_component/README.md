...
| `HostRequirements`         | POD record         | Generic host-needs declaration (RT policy, pinning, threads) published via registerData under `HOST_REQUIREMENTS_NAME`; assessors discover publishers by registry scan | Publish: config-time                    |
