...
/** @test Table sizes match Config and are reasonable. */
TEST(DataSequence, ConstantSanity) {
  EXPECT_EQ(SEQUENCE_MAX_STEPS, system_core::data::Config::SEQUENCE_MAX_STEPS);
  EXPECT_EQ(SEQUENCE_TABLE_SIZE,
            system_core::data::Config::RTS_SLOT_COUNT + system_core::data::Config::ATS_SLOT_COUNT);
...
/* ----------------------------- advanceStep with chainId ----------------------------- */

/** @test NEXT completion returns chainId=0 (no chain). */
TEST(DataSequence, AdvanceNextNoChain) {
  DataSequence seq{};
  seq.stepCount = 2;
  ...
  std::uint16_t chainId = 99;
  system_core::data::advanceStep(seq, chainId);

  EXPECT_EQ(chainId, 0U);
  EXPECT_EQ(seq.currentStep, 1U);
}

/** @test GOTO_STEP completion jumps to target step, no chain. */
...
/** @test START_RTS completion advances linearly and sets chainTargetId. */
TEST(DataSequence, AdvanceStartRts) {
  DataSequence seq{};
  seq.stepCount = 2;
  seq.steps[0].onComplete = StepCompletionAction::START_RTS;
  seq.steps[0].chainTargetId = 47; // Sequence ID 47
  seq.currentStep = 0;
  seq.status = SequenceStatus::EXECUTING;
...
TEST(DataSequence, ActionDataPreserved) {
  ...
  float val = 42.0F;
  system_core::data::initCommandAction(seq.steps[0].action, 0x007800, ActionTrigger::IMMEDIATE, 0,
                                       0x01, &val, sizeof(float));
  seq.armed = true;
  ...
  EXPECT_EQ(a->commandOpcode, 0x01);

  float recovered = 0.0F;
  std::memcpy(&recovered, a->commandPayload.data(), sizeof(float));
...
/** @test abortSequence sets abortEventPending when abortEventId is configured. */
TEST(DataSequence, AbortSequenceSetsAbortEventPending) {
  DataSequence seq{};
  seq.stepCount = 1;
  seq.armed = true;
  seq.abortEventId = 99;
...
/** @test abortSequence does NOT set pending when abortEventId is 0. */
TEST(DataSequence, AbortSequenceNoPendingWhenNoAbortEvent) {
  DataSequence seq{};
  seq.stepCount = 1;
  seq.armed = true;
  seq.abortEventId = 0;
...
/** @test abortSequence on non-running sequence does nothing. */
TEST(DataSequence, AbortIdleSequenceNoPending) {
  DataSequence seq{};
  seq.abortEventId = 99;
...
/** @test Timeout with ABORT policy sets abortEventPending. */
TEST(DataSequence, TimeoutAbortSetsAbortEventPending) {
  DataSequence seq{};
  seq.stepCount = 1;
  seq.steps[0].delayCycles = 0;
  seq.steps[0].timeoutCycles = 1;
  seq.steps[0].onTimeout = StepTimeoutPolicy::ABORT;
  seq.armed = true;
  seq.abortEventId = 50;
  ...
  // Simulate timeout
  seq.status = SequenceStatus::WAITING;
  seq.steps[0].timeoutRemaining = 0;
  bool result = system_core::data::applyTimeoutPolicy(seq);
  EXPECT_FALSE(result);
  EXPECT_EQ(seq.status, SequenceStatus::TIMED_OUT);
  EXPECT_TRUE(seq.abortEventPending);
}

/** @test Timeout with SKIP policy does NOT set abortEventPending. */
TEST(DataSequence, TimeoutSkipNoPending) {
  DataSequence seq{};
  seq.stepCount = 2;
  seq.steps[0].delayCycles = 0;
  seq.steps[0].timeoutCycles = 1;
