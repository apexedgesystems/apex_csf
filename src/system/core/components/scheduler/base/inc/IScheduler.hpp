 ...
 *   - Suitable for both ApexExecutive (SchedulerBase) and McuExecutive (McuScheduler)
 *
 * RT Constraints:
 ...
 *   - McuScheduler (mcu/) - Static task table, no heap in hot path
 */
 ...
 *   - McuScheduler: Minimal for bare-metal MCUs (McuExecutive)
 */
class IScheduler {
public:
