#ifndef APEX_SIM_ELECTRONICS_CPU_INTEL4004_COMPONENTS_HPP
#define APEX_SIM_ELECTRONICS_CPU_INTEL4004_COMPONENTS_HPP
/**
...
inline ComponentClassification classifyComponents(const Intel4004Grid& grid) {
  ...
  std::unordered_set<mna::NetID> norOutputNets;
  for (std::size_t i = 0; i < grid.transistors_.size(); ++i) {
    const auto& t = grid.transistors_[i];
    if (t.isDiodeLoad) {
      mna::NetID outNet = (t.drain == vdd) ? t.source : t.drain;
      if (outNet != 0)
...
#endif // APEX_SIM_ELECTRONICS_CPU_INTEL4004_COMPONENTS_HPP
