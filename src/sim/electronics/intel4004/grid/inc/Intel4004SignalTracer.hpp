#ifndef APEX_SIM_ELECTRONICS_CPU_INTEL4004_SIGNALTRACER_HPP
#define APEX_SIM_ELECTRONICS_CPU_INTEL4004_SIGNALTRACER_HPP
/**
...
struct TraceNode {
  ...
  mna::NetID netId = 0;         ///< Internal net ID.
...
struct Intel4004SignalTracer {
  ...
  std::size_t traceFrom(const std::string& startNet, int maxDepth = 5) {
    ...
    std::queue<std::pair<mna::NetID, int>> bfs;
    bfs.push({startId, 0});
    ...
    while (!bfs.empty()) {
      ...
      for (std::size_t ti = 0; ti < grid_.transistors_.size(); ++ti) {
        ...
        // Check each terminal
        auto tryAdd = [&](mna::NetID neighbor, const std::string& via) {
...
private:
  ...
  std::unordered_map<mna::NetID, std::string> idToName_;
  std::vector<TraceNode> trace_;
  std::set<mna::NetID> visited_;
  std::vector<std::string> snapshotLabels_;
};

} // namespace sim::electronics::intel4004

#endif // APEX_SIM_ELECTRONICS_CPU_INTEL4004_SIGNALTRACER_HPP
