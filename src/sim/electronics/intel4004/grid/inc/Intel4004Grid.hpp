#ifndef APEX_SIM_ELECTRONICS_CPU_INTEL4004_INTEL4004GRID_HPP
#define APEX_SIM_ELECTRONICS_CPU_INTEL4004_INTEL4004GRID_HPP
/**
...
struct ResolvedTransistor {
  sim::electronics::mna::NetID drain;
  sim::electronics::mna::NetID gate;
  sim::electronics::mna::NetID source;
...
struct Intel4004Grid {
  ...
  std::unordered_map<std::string, sim::electronics::mna::NetID> netMap_;
  sim::electronics::mna::NetID vdd_ = 0;

  /* ----------------------------- External IO Nets ----------------------------- */

  sim::electronics::mna::NetID clk1Net_ = 0;
  sim::electronics::mna::NetID clk2Net_ = 0;
  std::array<sim::electronics::mna::NetID, 4> dataBusNets_{}; ///< D0-D3.
  std::array<sim::electronics::mna::NetID, 4> accNets_{};     ///< ACC.0-ACC.3.
  ...
  std::array<sim::electronics::mna::NetID, 8> timingNets_{};
  sim::electronics::mna::NetID syncNet_ = 0;      ///< SYNC: HIGH during A1-A3.
  sim::electronics::mna::NetID execCtrlNet_ = 0;  ///< ~(X31&~CLK2): execute-phase ctrl.
  sim::electronics::mna::NetID execCtrl2Net_ = 0; ///< ~(X21&~CLK2): another exec ctrl.
  ...
  struct CompoundTimingNets {
    mna::NetID inhX11X31Clk1 = 0; // (~INH)(X11+X31)CLK1
    mna::NetID pocClk2A32X12 = 0; // (~POC)&CLK2&SC(A32+X12)
    mna::NetID clk2A12M12 = 0;    // CLK2&SC(A12+M12)
    mna::NetID m12m22Clk1 = 0;    // M12+M22+CLK1~(M11+M12)
    mna::NetID scA22M22Clk2 = 0;  // SC(A22+M22)CLK2
    mna::NetID scM22Clk2 = 0;     // SC&M22&CLK2
    mna::NetID scM12Clk2 = 0;     // SC&M12&CLK2
    mna::NetID scA12Clk2 = 0;     // SC&A12&CLK2
    mna::NetID scA22 = 0;         // SC&A22
    mna::NetID inhX32Clk2 = 0;    // (~INH)&X32&CLK2
    mna::NetID pocClk2X12X32 = 0; // (~POC)CLK2(X12+X32)~INH
    mna::NetID jmsDcBbl = 0;      // CLK2(JMS&DC&M22+BBL(M22+X12+X22))
    mna::NetID scJinFin = 0;      // ((~SC)(JIN+FIN))CLK1(M11+X21~INH)
  } compNets_;

  /* ----------------------------- Internal State Nets ----------------------------- */

  sim::electronics::mna::NetID cyNet_ = 0;                                ///< CY (carry flag).
  std::array<std::array<sim::electronics::mna::NetID, 4>, 16> regNets_{}; ///< R0.0-R15.3.
  std::array<sim::electronics::mna::NetID, 12> pcNets_{};                 ///< PC0.0-PC0.11.
  ...
  void enableSparseMode(Circuit& circuit) {
    circuit.solver().setStatefulStampCallbackSparse(
        [this](sim::electronics::mna::MnaSystemSparse& mna, double /*time*/,
   ...
   * @return Final sim::electronics::transient::TransientState with node voltages.
   *
   ...
   * @return sim::electronics::mna::NetID, or 0 if not found.
   */
  [[nodiscard]] sim::electronics::mna::NetID findNet(const std::string& name) const {
  ...
  [[nodiscard]] double readNetVoltage(const std::vector<double>& voltages,
    ...
    sim::electronics::mna::NetID id = findNet(name);
...
protected:
  ...
  void driveNet(MnaSystemT& mna, sim::electronics::mna::NetID netId, bool high) const {
   ...
   */
  template <typename MnaSystemT>
  void driveBus(MnaSystemT& mna, const sim::electronics::mna::NetID* nets, std::size_t bits,
  ...
  static std::uint32_t readBus(const std::vector<double>& v,
                               const sim::electronics::mna::NetID* nets, std::size_t bits) {
    std::uint32_t val = 0;
    for (std::size_t i = 0; i < bits; ++i) {
...
protected:
  mutable std::unordered_set<mna::NetID>
...
protected:
  ...
  void allocateNets(Circuit& circuit, const Intel4004Netlist& netlist) {
    ...
    for (const auto& name : netlist.uniqueNets) {
      ...
      sim::electronics::mna::NetID id = circuit.addNet(name).id;
      netMap_[name] = id;
   ...
   * gate is a different sim::electronics::mna::NetID from VDD.
   *
   * Algorithm:
  ...
  void identifyStaticVddNets() {
    ...
    while (changed) {
      ...
      std::unordered_set<sim::electronics::mna::NetID> candidates;
      for (const auto& t : transistors_) {
        if (!t.isDiodeLoad) {
          continue;
        }
        sim::electronics::mna::NetID output = (t.drain == vdd_) ? t.source : t.drain;
  ...
  sim::electronics::mna::NetID resolveNet(const std::string& name) const {
    auto it = netMap_.find(name);
    return (it != netMap_.end()) ? it->second : 0;
  }
  ...
  void registerStamps(Circuit& circuit) {
    circuit.addStamp(
        [this](sim::electronics::mna::MnaSystem& mna, double, const std::vector<double>&) {
          mna.addVoltageSource(vdd_, Circuit::ground(), VDD_VOLTAGE);
        });

    circuit.addStamp([this](sim::electronics::mna::MnaSystem& mna, double /*time*/,
                            const std::vector<double>& prevV) {
      stampTransistors(mna, prevV);
      stampParasiticCaps(mna, prevV);
    });

    circuit.addStamp([this](sim::electronics::mna::MnaSystem& mna, double,
  ...
  void stampParasiticCaps(MnaSystemT& mna, const std::vector<double>& prevV) const {
    ...
    for (sim::electronics::mna::NetID n = 1; n < N; ++n) {
      if (n == vdd_) {
        continue;
      }
  ...
  void runSubSteps(Circuit& circuit, sim::electronics::transient::TransientState& state,
   ...
   */
  void runByteLoop(Circuit& circuit, sim::electronics::transient::TransientState& state,
...
#endif // APEX_SIM_ELECTRONICS_CPU_INTEL4004_INTEL4004GRID_HPP
