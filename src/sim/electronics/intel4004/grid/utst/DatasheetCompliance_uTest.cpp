 ...
 */
TEST(DatasheetCompliance, NorGateVolFormula) {
  using Grid = sim::electronics::intel4004::Intel4004GridLevel1;
 ...
 */
TEST(DatasheetCompliance, NorGateVohWithinDatasheetRange) {
 ...
 */
TEST(DatasheetCompliance, ReadThresholdIsInForbiddenZone) {
  const double readThreshold = VDD / 2.0;

  EXPECT_GT(readThreshold, DATASHEET_VIL_MAX)
      << "Read threshold above VIL_MAX (otherwise we'd false-positive on VIL signals)";
  EXPECT_LT(readThreshold, DATASHEET_VIH_MIN)
  ...
  std::printf("  Read threshold = %.2f V (datasheet forbidden zone: %.2f-%.2f V)\n",
              readThreshold, DATASHEET_VIL_MAX, DATASHEET_VIH_MIN);
 ...
 */
TEST(DatasheetCompliance, OutputDriverProducesDriverCurrent) {
  using Grid = sim::electronics::intel4004::Intel4004GridLevel1;
 ...
 */
TEST(DatasheetCompliance, ScaledThresholdRatioMatchesRealSilicon) {
  using Grid = sim::electronics::intel4004::Intel4004GridLevel1;
  const double scaledRatio = Grid::VTH_ENH / VDD;
  const double realRatio = 3.7 / 15.0;  // approximate from Faggin docs

  EXPECT_NEAR(scaledRatio, realRatio, 0.02)
      << "Scaled Vt/VDD ratio must approximate real silicon's";

  std::printf("  Vt/VDD ratio: model=%.4f, real silicon~%.4f\n",
              scaledRatio, realRatio);
}
