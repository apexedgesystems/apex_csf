 ...
 */
TEST(Intel4004L2, MultiInstructionParityVsL0) {
 ...
 */
TEST(Intel4004L2, ProductionTests_AllRamIoOps) {
 ...
 */
TEST(Intel4004L2, ProductionTests_SubroutineRoundtrip) {
  // 0x000: LDM 5      ACC=5
 ...
 */
TEST(Intel4004L2, ProductionTests_NestedJms) {
 ...
 *       R0 wraps to 0.
 */
TEST(Intel4004L2, ProductionTests_IszLoop) {
 ...
 */
TEST(Intel4004L2, ProductionTests_BankSwitchedRam) {
  const std::vector<std::uint8_t> PROG = {
 ...
 */
TEST(Intel4004L2, ProductionTests_JcnAllConditions) {
 ...
 */
TEST(Intel4004L2, ProductionTests_BcdAdd) {
 ...
 */
TEST(Intel4004L2, FullCoverageParityVsL0) {
  ...
  while (cpu.pc < N) {
    const std::uint16_t pcBefore = cpu.pc;
    cpu.step();
    L0Snap s{};
    s.acc = static_cast<std::uint8_t>(cpu.accumulator);
    s.cy = cpu.carry;
  ...
  for (std::size_t b = 0; b < N; ++b) {
    ...
    if (l0Idx < l0Snaps.size() && (b + 1) == l0Snaps[l0Idx].bytesConsumed) {
      ...
      const std::uint8_t ramAt42 =
          (0x42 < grid.ramData_.size()) ? grid.ramData_[0x42] : 0;
      const std::uint8_t ramStat0 =
      ...
      bool match = a == l0.acc && cy == l0.cy && r0 == l0.regs[0] &&
                   ...
                   ramAt42 == l0.ramAt42 && ramStat0 == l0.ramStatusAt0;
      std::printf("  %2zu     %3zu    0x%02X   L0[%X %d %X %X %X %X %X %X]  "
                  ...
                  a, cy ? 1 : 0, r0, r1, r4, r5, ramAt42, ramStat0,
                  match ? "PASS" : "FAIL");
      ++totalChecks;
      if (match) ++passed;
      ...
      EXPECT_EQ(ramAt42, l0.ramAt42) << "step " << l0Idx << " ramData[0x42]";
      EXPECT_EQ(ramStat0, l0.ramStatusAt0) << "step " << l0Idx << " ramStatus[0]";
      ++l0Idx;
    }
  }
 ...
 *       matches.
 */
TEST(Intel4004L2, JunJmsIszFinVsL0) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
  constexpr std::size_t WARMUP = 32;
