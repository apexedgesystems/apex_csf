...
TEST(Intel4004Grid, BuildCircuit) {
...
TEST(Intel4004Grid, NetAllocation) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
  Intel4004Grid grid;
...
TEST(Intel4004Grid, TransistorResolution) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
  Intel4004Grid grid;
  grid.buildCircuit(NETLIST);
...
TEST(Intel4004Grid, StampRegistration) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
  Intel4004Grid grid;
  auto circuit = grid.buildCircuit(NETLIST);
...
TEST(Intel4004Grid, DcOperatingPointConverges) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
  Intel4004Grid grid;
  auto circuit = grid.buildCircuit(NETLIST);
...
TEST(Intel4004Grid, DcVddVoltage) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
  Intel4004Grid grid;
  auto circuit = grid.buildCircuit(NETLIST);
  grid.enableSparseMode(circuit);
...
TEST(Intel4004Grid, DcGroundVoltage) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
  Intel4004Grid grid;
  auto circuit = grid.buildCircuit(NETLIST);
  grid.enableSparseMode(circuit);
...
TEST(Intel4004Grid, DcAddAccControl) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
...
TEST(Intel4004Grid, DcVoltageDistribution) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
  Intel4004Grid grid;
  auto circuit = grid.buildCircuit(NETLIST);
  grid.enableSparseMode(circuit);
...
TEST(Intel4004Grid, SimulateNopChain) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
  Intel4004Grid grid;
  auto circuit = grid.buildCircuit(NETLIST);
  grid.enableSparseMode(circuit);
...
TEST(Intel4004Grid, ExternalNetsCached) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
  Intel4004Grid grid;
  grid.buildCircuit(NETLIST);
...
TEST(Intel4004Grid, SingleByteFetch) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
  Intel4004Grid grid;
  auto circuit = grid.buildCircuit(NETLIST);
  grid.enableSparseMode(circuit);
...
TEST(Intel4004Grid, TimingGeneratorStartup) {
  const auto NETLIST = loadSpiceNetlist(SPICE_PATH);
