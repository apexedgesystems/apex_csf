...
TEST(MosfetBsim3Test, SmoothAcrossThreshold) {
  ...
  for (std::size_t i = 1; i < ids.size(); ++i) {
    ...
    EXPECT_LT(REL, 10.0) << "id jump at Vgs=" << (p.Vth0 - 0.2 + 0.01 * i)
                         << " is too large (kink)";
  }
}
...
TEST(MosfetBsim3Test, GmConsistentWithNumericalDerivative) {
  ...
  for (double vgs : {0.5, 1.0, 1.17, 1.5, 2.0, 3.0}) {
    ...
    EXPECT_NEAR(GM_MODEL, GM_REF, std::max(1e-9, 1e-4 * std::fabs(GM_REF))) << "vgs=" << vgs;
  }
}
 ...
 *        only the constant overlap caps. */
TEST(MosfetBsim3MeyerTest, CutoffRegion_GateBulkDominates) {
  MosfetBsim3Params p;
  p.K1 = 0.0;
  p.eta0 = 0.0; // disable body/DIBL for clean check
...
TEST(MosfetBsim3MeyerTest, LinearToSaturationContinuity) {
  ...
  EXPECT_NEAR(C_LIN_NEAR_SAT.Cgs, C_SAT.Cgs, 1e-12) << "Cgs continuous across linear/sat boundary";
  EXPECT_NEAR(C_LIN_NEAR_SAT.Cgd, C_SAT.Cgd, 1e-12) << "Cgd continuous across linear/sat boundary";
}
...
TEST(MosfetBsim3Test, BodyTransconductancePositiveInActiveRegion) {
  ...
  EXPECT_GT(GMB, 0.0) << "Expected dId/dVbs > 0 for NMOS in active region with K1>0; got " << GMB;
}
...
[[maybe_unused]] constexpr double KP = 5e-3;
[[maybe_unused]] constexpr double VTH_ENH = 1.17;
[[maybe_unused]] constexpr double VTH_DEP = -0.17;
constexpr double LAMBDA = 0.03;
[[maybe_unused]] constexpr double VDD = 5.0;
[[maybe_unused]] constexpr double WL_DEP = 0.10;
[[maybe_unused]] constexpr double WL_ENH = 3.23;
...
void stampPmosBsim3(MnaSystemSparse& mna, NetID drain, NetID gate, NetID source,
  ...
  const double ID = MosfetBsim3::current(VGS_M, VDS_M, VBS, P);
  const double GM = MosfetBsim3::transconductance(VGS_M, VDS_M, VBS, P);
  const double GDS_RAW = MosfetBsim3::outputConductance(VGS_M, VDS_M, VBS, P);
