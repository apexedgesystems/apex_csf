 ...
 *
 * Usage:
 *   ./SimDynamicsWrench_PTEST --gtest_list_tests
 *   ./SimDynamicsWrench_PTEST --profile perf --gtest_filter="*Wrench*"
 */
...
PERF_TEST(WrenchAccumulatorResult, Throughput) {
  UB_PERF_GUARD(perf);

  const Vec3 cg{0.3, 0.0, 0.1};
  const AppliedWrench gravity{Vec3{0.0, 0.0, 1.3e5}, cg, Vec3{}};
  const AppliedWrench aero{Vec3{-4.0e4, 0.0, -2.0e5}, Vec3{0.6, 0.0, 0.0}, Vec3{}};
  const AppliedWrench thrustL{Vec3{6.0e4, 0.0, 0.0}, Vec3{-3.0, -4.0, 1.0}, Vec3{}};
  const AppliedWrench thrustR{Vec3{6.0e4, 0.0, 0.0}, Vec3{-3.0, 4.0, 1.0}, Vec3{}};
  const AppliedWrench surface{Vec3{}, Vec3{}, Vec3{0.0, 1.5e4, 0.0}}; // pure couple
  ...
  auto result = perf.throughputLoop(
      [&] {
        const Wrench fm = acc.resultAbout(cg);
        sink += fm.force.x + fm.moment.y;
      },
      "wrench_accumulate");

  std::printf("\n[WrenchAccumulator] resultAbout (5 loads): %.0f stacks/s (%.4f us/stack)\n",
