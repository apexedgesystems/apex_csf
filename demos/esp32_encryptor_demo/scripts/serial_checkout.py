#!/usr/bin/env python3
"""
Firmware checkout script for Arduino Nano ESP32 (esp32_encryptor_demo).
...
Usage:
    python3 demos/esp32_encryptor_demo/scripts/serial_checkout.py
    python3 demos/esp32_encryptor_demo/scripts/serial_checkout.py --data-port /dev/ttyUSB1
    python3 demos/esp32_encryptor_demo/scripts/serial_checkout.py --cmd-port /dev/ttyACM1
    python3 demos/esp32_encryptor_demo/scripts/serial_checkout.py --verbose

Port assignments:
    Data channel:    UART0 via FTDI adapter  (/dev/ftdi_0)
    Command channel: USB CDC via native USB-C (/dev/esp32_0)

Checkout groups:
     1. Connection      - Serial ports accessible
     2. Data Channel    - SLIP framing, CRC validation, encrypt pipeline
     3. Key Store       - NVS-based key provisioning and management
    ...
    10. Overhead        - CCOUNT cycle tracking at 240 MHz
...
Prerequisites:
    ...
    - FTDI adapter connected to UART0 (D1/TX GPIO43, D0/RX GPIO44)
    - ESP32 USB-C cable connected (native USB CDC for command channel)
...
# ESP32-S3 CCOUNT frequency (Xtensa LX7 @ 240 MHz)
CCOUNT_FREQ_HZ = 240_000_000
...
def check_encrypt_basic(data_ser: serial.Serial) -> bool:
    ...
    plaintext = b"Hello ESP32 Encryptor!"
...
# =============================================================================
# Checkout Group 10: Overhead (CCOUNT cycle counter @ 240 MHz)
# =============================================================================
...
def check_overhead_query(cmd_ser: serial.Serial) -> bool:
    ...
    if count > 0 and budget > 0:
        ...
        print(
            ...
            f"last={last} cycles ({util:.1f}% of {budget})"
        )
        return True
...
def check_overhead_reset(cmd_ser: serial.Serial) -> bool:
    ...
    # Count will be nonzero (ticks accumulate during command round-trip)
    # At 100 Hz, expect ~100-300 samples for typical UART latency
    if count < 500:
...
def check_overhead_idle(cmd_ser: serial.Serial) -> bool:
    """Let firmware run for 1 second and verify max < budget."""
    send_command(cmd_ser, CMD_OVERHEAD_RESET)
    time.sleep(1.0)
    ...
    print(
        f"  PASS  Idle overhead: max={maximum} cycles "
...
def check_fastforward_overhead(cmd_ser: serial.Serial) -> bool:
    ...
    print(f"         Max achievable rate: ~{max_rate:.0f} Hz " f"(last={last} cycles @ 240 MHz)")
    return True
...
def run_checkout(data_port: str, cmd_port: str, baud: int) -> int:
    ...
    print("ESP32 Encryptor Checkout (ESP32-S3)")
    ...
    test_groups = [
        ...
        (
            ...
            "cmd",
            [
                ("Overhead query", lambda s: check_overhead_query(s)),
                ...
                ("Fast-forward of", lambda s: check_fastforward_off(s)),
            ],
        ),
        (
            "Idle",
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ESP32 Encryptor firmware checkout (ESP32-S3)")
    parser.add_argument(
        "--data-port",
    ...
    parser.add_argument(
        ...
        help="Command channel port - USB CDC (default: %(default)s)",
    )
