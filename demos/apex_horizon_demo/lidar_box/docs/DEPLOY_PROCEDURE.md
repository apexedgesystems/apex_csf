...
consumer (e.g. a visualization consumer scene implementing the wire format) runs on the host; the
...
T=tools/rust/target/debug ; TP=demos/apex_horizon_demo/lidar_box/tprm
...
docker compose run --rm dev-cuda ./build/hosted-x86_64-debug/bin/ApexLidarBoxDemo \
  --config demos/apex_horizon_demo/lidar_box/tprm/master.tprm \
