...
#ifndef APEX_CDEF_AUTO_SPEC_SENSOR_SPEC_BASE_HPP
#define APEX_CDEF_AUTO_SPEC_SENSOR_SPEC_BASE_HPP
...
template <typename TDerived, typename TBase> class SpecSensorSpecBase : public TBase {
public:
using TBase::TBase;

  enum class TaskUid : std::uint8_t {
    STEP = 1, ///< 50 Hz model step (drift + noise physics).
  };
  ...
  bool loadTprm(const std::filesystem::path& tprmDir) noexcept override {
    ...
    if (std::filesystem::exists(PATH)) {
      loaded = paramBank_.load(
                   PATH, this->fullUid(),
                   [this](const SpecSensorTunableParams& p) noexcept { return validateParams(p); },
...
[[nodiscard]] virtual std::uint8_t onInit() noexcept { return 0; }
  ...
  [[nodiscard]] virtual bool validateParams(const SpecSensorTunableParams&) noexcept { return true; }
...
#endif // APEX_CDEF_AUTO_SPEC_SENSOR_SPEC_BASE_HPP
