 ...
 *
 * The minimal component set around the spec-born SpecSensor: the
 * sensor itself (SW_MODEL, fullUid=0xD400) and SystemMonitor
 * (SUPPORT, fullUid=0xC800) for health telemetry. Pure SIL on any
 * POSIX host; the demo exists to prove the spec-driven path -- every
 * command the checkout drives dispatches through generated code.
 */
