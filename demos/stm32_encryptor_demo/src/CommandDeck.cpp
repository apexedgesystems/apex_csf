 ...
 * @brief Command channel handler implementation.
 *
 * Flow per poll():
 *  1. Read available bytes from UART2 RX buffer
...
/// Chunk size for reading from UART2 RX buffer per poll().
...
void CommandDeck::handleKeyStoreWrite(const uint8_t* payload, size_t len) noexcept {
  // Payload: slot(1) + key(32) = 33 bytes
...
void CommandDeck::handleKeyStoreErase() noexcept {
  ...
  // Invalidate engine state: switch to RANDOM, clear cached key
  engine_.unlock();
...
void CommandDeck::handleKeyStoreStatus() noexcept {
  ...
  payload[1] = static_cast<uint8_t>(BITMAP & 0xFF);        // LE low byte
  payload[2] = static_cast<uint8_t>((BITMAP >> 8) & 0xFF); // LE high byte
...
void CommandDeck::handleIvStatus() noexcept {
  ...
  // Append frame count as little-endian uint32
...
void CommandDeck::handleStats() noexcept {
  ...
  // All fields as little-endian uint32 (Cortex-M4 is native LE)
...
void CommandDeck::handleOverhead() noexcept {
  ...
  // Payload: last(4) + min(4) + max(4) + count(4) + budget(4) + flags(1) = 21 B
  uint8_t payload[21];
