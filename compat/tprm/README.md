...
| `payloads/*.bin`         | Format A v4: 28-byte prelude (magic APV4, version, 64-bit size, fullUid, layout hash, CRC-32) + body |
| `payloads/*_raw.bin`     | The unstamped serialization -- the form sequence entries pack                                  |
| `archives/basic.tprm`    | Format B: a packed archive (v4 component entries + a raw RTS-reserved entry)                   |
...
- **C++ runtime**: `GoldenVectors_uTest.cpp` verifies the same files
  through the v4 payload reader and `PackedTprmReader` and asserts
