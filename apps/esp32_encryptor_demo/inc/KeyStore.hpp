#ifndef APEX_ESP32_ENCRYPTOR_KEY_STORE_HPP
#define APEX_ESP32_ENCRYPTOR_KEY_STORE_HPP
/**
 * @file KeyStore.hpp
 * @brief NVS-backed AES-256 key storage for the esp32_encryptor_demo.
 *
 * Manages 16 key slots (32 bytes each) via NVS (Non-Volatile Storage).
 * Keys are cached in RAM for RT-safe read access. Write and erase
 * operations are blocking (NVS flash programming).
 *
 * NVS layout (namespace "encryptor", key "keys"):
...
/**
 * @class KeyStore
 * @brief NVS-persistent key storage with RAM cache.
 *
...
class KeyStore {
public:
  /// NVS uses a single virtual page (page 0 maps to the full blob).
...
#endif // APEX_ESP32_ENCRYPTOR_KEY_STORE_HPP
