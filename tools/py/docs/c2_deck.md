...
Generate a consolidated command/telemetry (C2) deck from Apex struct dictionaries.
...
c2-deck --db build/native-linux-debug/apex_data_db | less

# Write deck to file
c2-deck --db build/native-linux-debug/apex_data_db --output ApexHilDemo-deck.md

# Run apex-data-db first if dictionaries are not yet generated
make apex-data-db
c2-deck --db build/native-linux-debug/apex_data_db --output deck.md
```
...
- [c2_sdk_package.sh](../../../tools/sh/docs/c2_sdk_package.md) -- Bundles struct dictionaries and a generated deck into a deployable SDK package.
